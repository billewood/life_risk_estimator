...
class DataManager:
    ...
    def get_data_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        if os.path.exists(cdc_file):
            ...
            status['cdc_cause_data'] = {
                'available': False,
                'reason': 'File not found - automated fetch failed; see DATA_DOWNLOAD_GUIDE.md for manual fallback'
            }
    ...
    def update_data(self, source: str = "all", force: bool = False):
        ...
        if source in ["all", "cdc"]:
            print("CDC data: attempting automated fetch; if blocked, see DATA_DOWNLOAD_GUIDE.md for manual fallback")
        
        if source in ["all", "gbd"]:
            print("GBD data: attempting automated fetch; if blocked, see DATA_DOWNLOAD_GUIDE.md for manual fallback")
