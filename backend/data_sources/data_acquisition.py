...
class DataAcquisition:
    ...
    def download_ssa_life_tables(self, force_download: bool = False) -> pd.DataFrame:
        """
        Preferred: load packaged CSV shipped with the app.
        Optional: if force_download=True, fetch from HTML and update the packaged CSV/metadata.
        """
        ssa_file = f"{self.data_dir}/ssa_life_tables_2021.csv"
        metadata_file = f"{self.data_dir}/ssa_life_tables_metadata.json"

        # 1) Use packaged CSV if present (no network)
        if not force_download and os.path.exists(ssa_file):
            try:
                df = pd.read_csv(ssa_file)
                print(f"✓ Loaded packaged SSA CSV: {ssa_file}")
                ...
                data_logger.log_import(
                    source_id=source_id,
                    file_path=ssa_file,
                    data_hash=data_hash,
                    record_count=len(df),
                    import_notes="Packaged CSV load"
                )
                return df
            except Exception as e:
                print(f"Packaged SSA CSV exists but failed to load ({e}); attempting metadata/cached flow...")

        # 2) Use cached CSV + metadata if both present and recent
        ...
        # 3) If explicitly requested, fetch from HTML
        print("Downloading SSA life tables from HTML (force_download=True)...")
    ...
    def download_cdc_cause_data(self) -> pd.DataFrame:
        """
        Preferred: load packaged CSV shipped with the app.
        If not present, fall back to manually downloaded file.
        """
        print("Processing CDC cause of death data...")

        packaged_file = f"{self.data_dir}/cdc_cause_death_2022.csv"
        manual_file = f"{self.data_dir}/cdc_cause_death_manual.csv"

        load_order = [packaged_file, manual_file]
        for path in load_order:
            if os.path.exists(path):
                which = "Packaged" if path == packaged_file else "Manual"
                print(f"Found {which} CDC data: {path}")
                df = pd.read_csv(path)
                ...
                import_id = data_logger.log_import(
                    ...
                    data_hash=data_hash,
                    record_count=len(df),
                    import_notes=("Packaged CSV load" if path == packaged_file else "Manual CSV load")
                )
                print(f"CDC cause data processed successfully (Import ID: {import_id})")
                return df

        raise FileNotFoundError(
            "CDC cause data not found. Expected packaged file 'cdc_cause_death_2022.csv' or manual 'cdc_cause_death_manual.csv' in backend/data_sources"
        )
    
    def download_gbd_risk_factors(self) -> Dict[str, Any]:
        """
        Preferred: load packaged JSON shipped with the app.
        If not present, fall back to manually downloaded JSON.
        """
        print("Processing GBD risk factor data...")

        packaged_file = f"{self.data_dir}/gbd_risk_factors_2019.json"
        manual_file = f"{self.data_dir}/gbd_risk_factors_manual.json"

        for path in [packaged_file, manual_file]:
            if os.path.exists(path):
                which = "Packaged" if path == packaged_file else "Manual"
                print(f"Found {which} GBD data: {path}")
                ...
                import_id = data_logger.log_import(
                    ...
                    import_notes=("Packaged JSON load" if path == packaged_file else "Manual JSON load")
                )
                print(f"GBD risk factors processed successfully (Import ID: {import_id})")
                return data

        raise FileNotFoundError(
            "GBD risk factors not found. Expected packaged file 'gbd_risk_factors_2019.json' or manual 'gbd_risk_factors_manual.json' in backend/data_sources"
        )
    ...
    def download_all_sources(self, force_download_ssa: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            results['ssa_life_tables'] = self.download_ssa_life_tables(force_download=force_download_ssa)
            results['cdc_cause_data'] = self.download_cdc_cause_data()
            results['gbd_risk_factors'] = self.download_gbd_risk_factors()
            # ePrognosis download not invoked by default
