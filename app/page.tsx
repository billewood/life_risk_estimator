...
export default function Home() {
  ...
  const [method, setMethod] = useState<'ssa' | 'nchs'>('ssa')
  const [showLifeExpectancyInfo, setShowLifeExpectancyInfo] = useState(false)
  const [showMortalityInfo, setShowMortalityInfo] = useState(false)
  const [showCVDInfo, setShowCVDInfo] = useState(false)
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-t pt-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-700">Calculation Method</h3>
                  <p className="text-sm text-gray-500">Choose between different data sources for comparison</p>
                </div>
              </div>
              ...
              <div className="flex space-x-4">
                <label className="flex items-center">
                  <input
                    ...
                    value="ssa"
                    checked={method === 'ssa'}
                    onChange={(e) => setMethod(e.target.value as 'ssa' | 'nchs')}
                    className="mr-2"
                  />
                  <div>
                    <div className="font-medium">SSA 2021 Data</div>
                    <div className="text-sm text-gray-500">Social Security Administration life tables</div>
                  </div>
                </label>
                ...
                <label className="flex items-center">
                  ...
                  />
                  <div>
                    <div className="font-medium">NCHS 2023 Data</div>
                    <div className="text-sm text-gray-500">National Center for Health Statistics with race/ethnicity breakdowns</div>
                  </div>
                </label>
              </div>
            </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-blue-800">
                    Calculation Method: {result.metadata?.method_used === 'nchs' ? 'NCHS 2023 Data' : 'SSA 2021 Data'}
                  </h3>
                  <p className="text-sm text-blue-700 mt-1">
                    {result.metadata?.method_used === 'nchs' 
                      ? 'Using National Center for Health Statistics life tables with race/ethnicity breakdowns'
                      : 'Using Social Security Administration life tables'
                    }
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white rounded-lg shadow-md p-6 text-center">
                ...
                {showLifeExpectancyInfo && (
                  <div className="mt-3 p-3 bg-blue-50 rounded-lg text-left text-sm">
                    <p className="font-medium text-blue-800 mb-1">Methodology:</p>
                    <p className="text-blue-700">
                      {result.metadata?.method_used === 'nchs' 
                        ? 'Based on NCHS 2023 life tables with race/ethnicity breakdowns. Uses actual mortality rates (qx) and life expectancy (ex) from official CDC data.'
                        : 'Based on SSA 2021 period life tables. Uses official Social Security Administration mortality data and life expectancy calculations.'
                      }
                    </p>
                  </div>
                )}
               ...
               <div className="bg-white rounded-lg shadow-md p-6 text-center">
                 <div className="flex items-center justify-center mb-2">
                   ...
                   <button
                     className="ml-2 text-gray-400 hover:text-gray-600"
                     title="Click for methodology details"
                     onClick={() => setShowMortalityInfo(!showMortalityInfo)}
                   >
                     <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                       <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                     </svg>
                 ...
                 {showMortalityInfo && (
                   <div className="mt-3 p-3 bg-red-50 rounded-lg text-left text-sm">
                     <p className="font-medium text-red-800 mb-1">Methodology:</p>
                     <p className="text-red-700">
                       {result.metadata?.method_used === 'nchs' 
                         ? 'Uses actual 1-year mortality rates (qx) from NCHS 2023 life tables, adjusted for your risk factors using relative risks from Global Burden of Disease data.'
                         : 'Uses actual 1-year mortality rates (qx) from SSA 2021 life tables, adjusted for your risk factors using relative risks from Global Burden of Disease data.'
                       }
                     </p>
                   </div>
                 )}
              ...
              <div className="bg-white rounded-lg shadow-md p-6 text-center">
                <div className="flex items-center justify-center mb-2">
                  ...
                  <button
                    className="ml-2 text-gray-400 hover:text-gray-600"
                    title="Click for methodology details"
                    onClick={() => setShowCVDInfo(!showCVDInfo)}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                    </svg>
                ...
                {showCVDInfo && (
                  <div className="mt-3 p-3 bg-purple-50 rounded-lg text-left text-sm">
                    <p className="font-medium text-purple-800 mb-1">Methodology:</p>
                    <p className="text-purple-700">
                      Uses Pooled Cohort Equations (PCE) - the gold standard for cardiovascular risk assessment. 
                      Based on 9 factors: age, sex, race, total cholesterol, HDL cholesterol, systolic blood pressure, 
                      blood pressure treatment status, smoking status, and diabetes status. Validated for ages 40-79.
                    </p>
                  </div>
                )}
              </div>
